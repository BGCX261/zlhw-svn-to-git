...
public abstract class BaseTable extends BasicModel implements XMLEcho, Serializable,Cloneable {
	...
	public Element getElement() {
		...
		System.out.println("正在映射:"+this.getTableName());
...
//						trueField.setMethod());
	...
	public void echo() {
		...
		System.out
				.println("\t//自动生成的模型POJO-----------------------------------------");
		StringBuffer tableComment = new StringBuffer("注释：").append(this.getTableName())
				.append(":").append(this.getComment());
		this.getComment();
		// 打印表的说明性注释
		System.out.println("\t//" + tableComment.toString());
		// 打印主键注释
		System.out.println("\t//主键" + this.getPrimaryField().getFieldName()
				+ "类型：" + this.getPrimaryField().getType().getSimpleName());
		printFieldList();
	}

	public void addUpdateProperties() {
		// 添加表到property
		propertiesOperator rc = new propertiesOperator("conf/beans.properties");// 相对路径
		rc.setValue(this.getClass().getName(), this.getTableName());
		rc.saveFile();
		System.out.println("\t//已在beans.properties产生/更新当前类-------------------------------");
	}
