...
import com.ZLHW.base.factory.FlexControl;
...
//	}
	...
	public Map login(Admin user) throws BaseErrorModel{
		...
		if(l.size()==0){
			...
			if(user.getPassword().equals(admin.getPassword())){
				...
				FlexControl.userMap.put(admin.getAccount(), this.getFlexSession().getId());
				map.put("admin", admin);
			}else{
	...
	public void loginOut(){
		System.out.println(this.getAdmin().getAccount()+"退出");
	 ...
	 * @return
	 */
	public List<Author> getMenu(){
		//获取当前用户权限
		List<Author> authors = new ArrayList();
		//获取当前用户权限明细
		List<AuthorDetail>   authorDetails = this.getMenuDetailsByAdmin(this.getAdmin());
	...
	public List<AuthorDetail> getAuthorDetails(){
		return getAuthorDetailsByAdmin(this.getAdmin());
	}
	//获取当前用户初始化环境
	public Map getInitInfo() {
		// TODO Auto-generated method stub
		Map map = new HashMap();
		try
		{
			//获取当前用户权限
			List<Author> authors = this.getMenu();
